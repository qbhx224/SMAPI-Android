    ...
    *********/
    ...
    public PlatformAssemblyMap(Platform targetPlatform, HashSet<string> removeAssemblyNames, IReadOnlyList<Assembly> targetAssemblies)
    {
        ...
        this.TargetModules = this.Targets.ToDictionary(
            ...
            assembly =>
            {
                // 不用 Module.FullyQualifiedName（blob 加载的程序集可能无 .dll 后缀），
                // 用程序集名 + ".dll" 拼接（克隆到 Stardew Assemblies 的文件名）
                string assemblyFileName = assembly.GetName().Name + ".dll";
                string assemblyFullPath = Path.Combine(Constants.GamePath, assemblyFileName);
